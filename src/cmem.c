...
/**
 * @brief Allocates raw system memory with optional alignment, HugePages, Guard Pages, and NUMA
 * binding.
...
/**
 * @brief Frees raw system memory, respecting static buffer, custom allocator, HugePages, and Guard
 * Pages modes.
 * @param pool Pointer to the memory pool
 ...
 */
static inline bool percpu_push(memory_pool_t* pool, int cpu, uint8_t class_idx,
                               mp_slab_slot_t* slot)
{
    if (!pool->percpu_freelists || cpu < 0 || cpu >= pool->num_cpus)
        return false;
...
size_t mp_export_pprof(memory_pool_t* pool, char* out_buf, size_t max_len)
{
    ...
    int n = snprintf(out_buf, max_len,
                     ...
                     "inuse_space: %zu\n"
                     "peak_space: %zu\n",
                     active_bytes, active, total_alloc,
                     total_alloc > 0 ? pool->stats.total_alloc_ops * sizeof(void*) : 0, active,
                     active_bytes, peak_bytes);
...
static void* mp_alloc_internal(memory_pool_t* pool, size_t size)
{
        ...
        pool->stats.active_bytes + size > pool->stats.max_memory_limit)
    {
        ...
        if (pool->gc_cb)
        {
            pool->gc_cb(pool, true, pool->stats.active_bytes, pool->stats.max_memory_limit,
                        pool->gc_user_data);
        }
        if (pool->eviction_cb)
        {
            pool->eviction_cb(pool, true, pool->stats.active_bytes, pool->stats.max_memory_limit,
...
void mp_free(memory_pool_t* pool, void* ptr)
{
    ...
    if (header->magic != MP_MAGIC_HEAD)
    {
        ...
        {
            pool->error_recovery_cb(pool, true, pool->stats.active_bytes,
                                    pool->stats.max_memory_limit, pool->error_recovery_user_data);
        }
        return;
    }
